...
// TRobots menus script 1.62
// Jonathan Valvano 11/28/2006
...
function options_hide()
{
	...
	{
		video_switch( slider_resolution,0,0);
...
function game_exit()
{
   dll_close(dll_handle);    // 6812 simulation
...
function menu_main()
{
	menu_txt1.STRING = quit_game_str;
	menu_txt2.STRING = options_str;
	menu_txt3.STRING = help_str;
	menu_txt4.STRING = resume_str;

	menu_do1 = exit_yesno;
	menu_do2 = game_options;
	menu_do3 = game_help;
	menu_do4 = _menu_clear;
...
var VideoResolution=8;
function game_resolution()
{
//	_toggle_video();
   VideoResolution = VideoResolution+1;
   if(VideoResolution==10)
   {
   	VideoResolution = 6;
	}
	if(video_switch(VideoResolution,16,1)==0)
	{
   	VideoResolution = 6;
   	video_switch(VideoResolution,16,1);
	} 
